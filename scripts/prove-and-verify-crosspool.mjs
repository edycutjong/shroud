...
const VERIFIER =
  process.env.CROSSPOOL_VERIFIER_ID ||
  "CDEEOEOHKMDVVIIWOKMQ6L4NZCXFAEDKYPCEI3GGRXUQPSOIMQJGQS6R";
...
async function run() {
  ...
  const input_nullifier = "7",
    input_secret = "8";
  const path_elements = ["111", "222"],
    path_indices = ["0", "0"];
  ...
  const fx_rate_numerator = "1100",
    fx_rate_denominator = "1000";
  const input_amount_a = "1000",
    output_amount_b = "1100",
    output_secret_b = "999";
  ...
  const { proof, publicSignals } = await snarkjs.groth16.fullProve(
    input,
    `${C}/cross_pool_js/cross_pool.wasm`,
    `${C}/cp_final.zkey`,
  );
  const vk = (
    await import(`${C}/crosspool_vk.json`, { with: { type: "json" } })
  ).default;
  console.log(
    "off-chain verify:",
