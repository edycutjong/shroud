...
const FED =
  process.env.FEDERATION_ID ||
  "CADP225KUYYG7IX42KGWFS4ED4YGDQVACDPTGXJY6PUSVLUKPIBYC7DT";
...
const viewUntil = (want, fn, ...args) => {
  for (let i = 0; i < 8; i++) {
    if (norm(view(fn, ...args)) === norm(want)) return want;
    sleep(1500);
  }
  return view(fn, ...args);
};
...
const op1 = addr(OP1_SRC),
  op2 = addr(OP2_SRC);
console.log(
  `Federation ${FED}  (N=${view("asp_count")} ASPs)  fresh root ${root.slice(0, 12)}…`,
);

const c1 = call(OP1_SRC, "attest_root", "--operator", op1, "--root", root);
console.log(
  `ASP#1 attests -> count=${c1}, approved=${view("is_root_approved", "--root", root)}`,
);
if (view("is_root_approved", "--root", root) === "true") {
  console.error("approved with only 1 attestation!");
  process.exit(1);
}
...
if (approved !== "true") {
  console.error("threshold not reached after 2 attestations!");
  process.exit(1);
}

const adopted = viewUntil(root, "get_root").replaceAll('"', "");
console.log(
  `Adopted live compliance root => ${adopted.slice(0, 12)}…  (matches: ${adopted === root})`,
);
if (adopted !== root) {
  console.error("approved root not adopted as live root!");
  process.exit(1);
}
